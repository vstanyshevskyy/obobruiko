---
title: GAD-7 (General Anxiety Disorder-7)
publishTime: 2021-12-27T07:54:58.454Z
showInLists: true
content:
  - fbDescription: Find out if you have anxiety with just 7 questions, and you'll
      receive results along with recommendations on what to do in your case.
    path: /questionnaires/gad-7
    image_alt: '"The Scream" by Norwegian artist Edvard Munch in 1893. The agonized
      face in the painting has become one of the most iconic images of art, seen
      as symbolizing the anxiety of the human condition.'
    questions:
      - text: 1. Feeling nervous, anxious, or on edge
        subscale: default
        minScore: 0
        answers:
          - text: Not at all
            value: 0
          - text: Several days
            value: 1
          - text: More than half the days
            value: 2
          - text: Nearly every day
            value: 3
      - text: 2. Not being able to stop or control worrying
        subscale: default
        answers:
          - text: Not at all
            value: 0
          - text: Several days
            value: 1
          - text: More than half the days
            value: 2
          - text: Nearly every day
            value: 3
      - text: 3. Worrying too much about different things
        subscale: default
        answers:
          - text: Not at all
            value: 0
          - text: Several days
            value: 1
          - text: More than half the days
            value: 2
          - text: Nearly every day
            value: 3
      - text: 4. Trouble relaxing
        subscale: default
        answers:
          - text: Not at all
            value: 0
          - text: Several days
            value: 1
          - text: More than half the days
            value: 2
          - text: Nearly every day
            value: 3
      - text: 5. Being so restless that its hard to sit still
        subscale: default
        answers:
          - text: Not at all
            value: 0
          - text: Several days
            value: 1
          - text: More than half the days
            value: 2
          - text: Nearly every day
            value: 3
      - text: 6. Becoming easily annoyed or irritable
        subscale: default
        answers:
          - text: Not at all
            value: 0
          - text: Several days
            value: 1
          - text: More than half the days
            value: 2
          - text: Nearly every day
            value: 3
      - text: 7. Feeling afraid as if something awful might happen
        subscale: default
        answers:
          - text: Not at all
            value: 0
          - text: Several days
            value: 1
          - text: More than half the days
            value: 2
          - text: Nearly every day
            value: 3
      - text: 8. How difficult have these problems made it to do work, take care of
          things at home, or get along with other people?
        subscale: default
        answers:
          - text: Not at all
            value: 0
          - text: Somewhat difficult
            value: 0
          - text: Very difficult
            value: 0
          - text: Extremely difficult
            value: 0
        minScore: 1
    instruction: How often have you been bothered by the following over the past 2 weeks?
    metaKeywords: anxiety, anxiety symptoms, symptoms of anxiety, anxiety test, am I
      anxious, generalised anxiety disorder, what is anxiety, gad
    copyButtonText: Copy Results
    results:
      - text: No anxiety. The obtained result is an assumption; for a more accurate
          diagnosis, consult a mental health professional—psychologist,
          psychotherapist, or psychiatrist.
        minScore: 0
        maxScore: 4
        subscale: default
      - text: Mild anxiety. Requires observation, completing the GAD-7 questionnaire
          every 2 weeks. Self-help under the guidance of a cognitive-behavioral
          therapy specialist. The obtained result is an assumption; for a more
          accurate diagnosis, consult a mental health professional—psychologist,
          psychotherapist, or psychiatrist. Do not forget to rule out medical
          causes of anxiety before diagnosing an anxiety disorder (for example,
          EKG for arrhythmias, TSH for thyroid disease).
        minScore: 5
        maxScore: 9
        subscale: default
      - text: Moderate anxiety. Requires cognitive-behavioral therapy, applied
          relaxation, or medication intervention. The obtained result is an
          assumption; for a more thorough diagnosis, consult a mental health
          professional—psychologist, psychotherapist, or psychiatrist. Do not
          forget to rule out medical causes of anxiety before diagnosing an
          anxiety disorder (for example, EKG for arrhythmias, TSH for thyroid
          disease).
        minScore: 10
        maxScore: 14
        subscale: default
      - text: Severe anxiety. Requires a combination of medication and psychotherapy
          (cognitive-behavioral or applied relaxation). The obtained result is
          an assumption; for a more comprehensive diagnosis, consult a mental
          health professional—psychologist, psychotherapist, or psychiatrist. Do
          not forget to rule out medical causes of anxiety before diagnosing an
          anxiety disorder (for example, EKG for arrhythmias, TSH for thyroid
          disease).
        minScore: 15
        maxScore: 21
        subscale: default
    metaDescription: Questionnaire for screening generalized anxiety. Check if you
      have generalized anxiety and receive recommendations on what to do in your
      case.
    resultTemplate: "Your results: {0}"
    title: "Anxiety test "
    language: EN
    image: /assets/uploads/edvard_munch-_1893-_the_scream-_oil-_tempera_and_pastel_on_cardboard-_91_x_73_cm-_national_gallery_of_norway.jpg
    copyResultsTemplate: |-
      GAD-7 (General Anxiety Disorder-7) from bobruiko.com on {0}

      RESULT SUMMARY:
      {1} point(s)
      {2}

      INPUTS:
      {3}
    publishTime: 2021-11-02T07:19:48.687Z
    useWhiteForNav: false
    description: GAD-7 (General Anxiety Disorder-7) – test that measures severity of
      anxiety symptoms.
  - fbDescription: Пройди науково валідований онлайн тест GAD-7 із 7 запитань, щоб
      оцінити рівень генералізованої тривожності, виявити симптоми та отримати
      персональні рекомендації для підтримки емоційного здоров’я.
    path: /questionnaires/gad-7-ua
    image_alt: «Крик» норвезького художника Едварда Мункa, 1893 рік. Обличчя на
      картині стало однією з найбільш іконічних картин мистецтва, розглядається
      як символ тривоги людського стану.
    questions:
      - text: 1. Ви нервували, відчували тривогу або були дуже напружені
        subscale: default
        answers:
          - text: Зовсім ні
            value: 0
          - text: Декілька днів
            value: 1
          - text: Більше половини днів
            value: 2
          - text: Майже щодня
            value: 3
        minScore: 0
      - text: 2. Ви не могли зупинити або контролювати своє хвилювання
        subscale: default
        minScore: 0
        answers:
          - text: Зовсім ні
            value: 0
          - text: Декілька днів
            value: 1
          - text: Більше половини днів
            value: 2
          - text: Майже щодня
            value: 3
      - text: 3. Ви занадто багато хвилювались про різні речі
        subscale: default
        minScore: 0
        answers:
          - text: Зовсім ні
            value: 0
          - text: Декілька днів
            value: 1
          - text: Більше половини днів
            value: 2
          - text: Майже щодня
            value: 3
      - text: 4. Вам було важко розслабитись
        subscale: default
        minScore: 0
        answers:
          - text: Зовсім ні
            value: 0
          - text: Декілька днів
            value: 1
          - text: Більше половини днів
            value: 2
          - text: Майже щодня
            value: 3
      - text: 5. Ви були настільки неспокійні, що було важко всидіти на одному місці
        subscale: default
        minScore: 0
        answers:
          - text: Зовсім ні
            value: 0
          - text: Декілька днів
            value: 1
          - text: Більше половини днів
            value: 2
          - text: Майже щодня
            value: 3
      - text: 6. Вас було легко розсердити чи роздратувати
        subscale: default
        minScore: 0
        answers:
          - text: Зовсім ні
            value: 0
          - text: Декілька днів
            value: 1
          - text: Більше половини днів
            value: 2
          - text: Майже щодня
            value: 3
      - text: 7. Відчували страх, ніби щось жахливе може трапитись
        subscale: default
        minScore: 0
        answers:
          - text: Зовсім ні
            value: 0
          - text: Декілька днів
            value: 1
          - text: Більше половини днів
            value: 2
          - text: Майже щодня
            value: 3
      - text: 8. Наскільки важко Вам було працювати, займатись домашніми справами або
          взаємодіяти з іншими людьми через ці труднощі?
        subscale: default
        minScore: 1
        answers:
          - text: Зовсім не важко
            value: 0
          - text: Дещо важко
            value: 0
          - text: Дуже важко
            value: 0
          - text: Надзвичайно важко
            value: 0
    instruction: >-
      1. Дай відповіді на 7 запитань про частоту симптомів тривожності за
      останні два тижні.

      2. Кожне питання має варіанти відповіді: «Зовсім ні», «Декілька днів», «Більше половини днів», «Майже щодня».

      3. Після завершення система підраховує бали та показує рівень тривожності.

      4. Отримай персональні рекомендації щодо подальших кроків.


      > *GAD-7 – це інструмент скринінгу і не замінює повноцінну діагностику.*




      ***Як часто за останні 2 тижні тебе турбували наступні труднощі?***
    metaKeywords: тест на тривогу, GAD-7, онлайн тест на тривожність, генералізована
      тривога, симптоми тривожності, тривожний розлад, оцінка тривоги,
      психологічний тест, перевір себе, чи є у мене тривога
    copyButtonText: Скопіювати результати
    results:
      - text: "Мінімальна тривога (0–4). Твій результат показує мінімальний рівень
          тривоги. Це означає, що хвилювання практично не заважає твоєму
          повсякденному життю і спеціальні втручання не потрібні. Пам’ятай: цей
          тест – орієнтовна оцінка, а не діагноз. Якщо відчуваєш непевність або
          тривога все ж турбує, завжди можеш звернутися до фахівця у сфері
          психічного здоровʼя – психолога, психотерапевта або психіатра."
        minScore: 0
        maxScore: 4
        subscale: default
      - text: "Легка тривога (5–9). Твій результат показує легкий рівень тривоги. Це
          нормально, багато людей відчувають подібне час від часу. Що може
          допомогти: повторювати тест через 2 тижні, щоб відстежувати свій стан;
          практикувати техніки самодопомоги або працювати з фахівцем у форматі
          когнітивно-поведінкової терапії (КПТ); ознайомитися зі статтею: «Не
          переймайся!» або знайомство з генералізованим тривожним розладом.
          Результат тесту – лише попередня оцінка. Для точнішої оцінки свого
          стану варто звернутися до фахівця у сфері психічного здоровʼя –
          психолога, психотерапевта або психіатра. Також не забудь виключити
          можливі фізіологічні причини тривоги (ЕКГ, ТТГ)."
        minScore: 5
        maxScore: 9
        subscale: default
      - text: "Помірна тривога (10–14). Твій результат свідчить про помірний рівень
          тривоги. Це сигнал приділити трохи більше уваги своєму психічному
          здоровʼю. Що можна зробити: когнітивно-поведінкова терапія, практична
          релаксація або консультація лікаря щодо можливого медикаментозного
          лікування. Це не діагноз. Для повної оцінки стану та підтримки свого
          психологічного здоров’я звернися до фахівця – психолога,
          психотерапевта чи психіатра. Також важливо виключити медичні причини
          тривоги (аритмії, порушення роботи щитоподібної залози тощо)."
        minScore: 10
        maxScore: 14
        subscale: default
      - text: "Важка тривога (15–21). Твій результат свідчить про високий рівень
          тривоги. Це означає, що тривога може суттєво впливати на твоє життя.
          Рекомендації: поєднання психотерапії (КПТ або прикладна релаксація) та
          медикаментозного лікування, якщо його призначає психіатр; обов’язково
          звернися до фахівця з психічного здоров’я – психолога, психотерапевта
          або психіатра – для детальної діагностики та підтримки. Пам’ятай:
          результат тесту – попередня оцінка, а не остаточний діагноз. Також
          важливо виключити можливі фізичні причини тривоги (наприклад, ЕКГ для
          аритмій, ТТГ для захворювань щитовидної залози)."
        minScore: 15
        maxScore: 21
        subscale: default
    metaDescription: Пройди науково валідований онлайн тест GAD-7 із 7 запитань, щоб
      оцінити рівень генералізованої тривожності, виявити симптоми та отримати
      персональні рекомендації для підтримки емоційного здоров’я.
    resultTemplate: "Результат: {0}"
    title: Тест на тривогу GAD-7 онлайн – оцінка генералізованої тривожності
    language: UK
    image: /assets/uploads/edvard_munch-_1893-_the_scream-_oil-_tempera_and_pastel_on_cardboard-_91_x_73_cm-_national_gallery_of_norway.jpg
    copyResultsTemplate: |-
      Опитувальник GAD-7 на bobruiko.com від {0}

      Результат:
      {1} бал(ів)
      {2}

      Відповіді:
      {3}
    publishTime: 2022-01-25T09:11:21.412Z
    useWhiteForNav: false
    description: >-
      Тривога трапляється у кожного, але інколи вона стає надто сильною і
      заважає відчувати спокій та радість у житті. Якщо ти помічаєш у себе
      постійне хвилювання, напруження, фізичні прояви тривожності або труднощі з
      концентрацією, пройти **науково валідований онлайн тест GAD-7** може стати
      першим кроком до розуміння свого стану. Пройди тест із 7 запитань, щоб
      дізнатися, чи є у тебе симптоми тривожності та наскільки вони виражені, і
      отримай персональні рекомендації щодо подальших дій.


      ### **Що таке тест GAD-7?**


      **GAD-7 (Generalized Anxiety Disorder-7)** – короткий і надійний опитувальник для скринінгу генералізованої тривожності. Він допомагає оцінити інтенсивність симптомів та зрозуміти, наскільки тривога впливає на повсякденне життя. Тест науково валідований і широко використовується психологами та психотерапевтами для первинної оцінки стану.


      ### **Як проходити тест?**
---
some text heree
