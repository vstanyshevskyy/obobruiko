---
title: Holmes-Rahe Life Stress Inventory
showInLists: true
content:
  - language: EN
    path: /questionnaires/holmes-rahe
    image: /assets/uploads/flaming-june.jpg
    image_alt: Flaming June, by Frederic Lord Leighton (1830-1896)
    title: 'Holmes and Rahe Stress Scale (SRRS): Measure Your Life Stress'
    pageTitle: 'Holmes and Rahe Stress Scale (SRRS): Measure Your Life Stress'
    fbTitle: ''
    description: |-
      ![Flaming June, by Frederic Lord Leighton (1830-1896)](/assets/uploads/flaming-june.jpg "Flaming June, by Frederic Lord Leighton (1830-1896)")

      If you’ve ever felt overwhelmed after a “good year” filled with big changes – like a new job, moving, or getting married – you’re not alone. The [**Holmes and Rahe Stress Scale**](https://en.wikipedia.org/wiki/Holmes_and_Rahe_Stress_Scale) explains why even positive life events can build up and impact your mental and physical health.

      Also known as the [**Social Readjustment Rating Scale**](https://pmc.ncbi.nlm.nih.gov/articles/PMC10727443/), this widely used stress measurement tool helps you understand how life changes contribute to your overall stress load.

      ***

      ## What Is the Holmes and Rahe Stress Scale?

      The Holmes and Rahe Stress Scale is a self-report questionnaire that measures your stress level based on major life events over the past 12 months.

      It was developed in 1967 by psychiatrists _Thomas Holmes_ and _Richard Rahe_, and first published in the Journal of Psychosomatic Research.

      ### Key Idea:

      Any significant life change – whether positive or negative – requires adjustment. And adjustment itself is a form of stress.

      That means events like:

      - Getting married
      - Starting a new job
      - Buying a home
      - Having a child

      …can be just as impactful as more obviously stressful experiences.

      The more changes you go through, the higher your cumulative stress level.

      ***

      ## How Does the Holmes and Rahe Stress Scale Work?

      The scale includes **43 common life events**, each assigned a score called _Life Change Units (LCUs)_. You check which events occurred in the past year, and their LCUs are automatically added to give you a **total stress score**. 

      The higher your total score, the greater your estimated stress load and, historically, the higher the risk of a stress‑related health problem in the next two years.

      Here’s how it works:
    instruction: |-
      * Please review the list of **43 life events** below.
      * **Click any event** that happened to you during the **previous 12 months** to select it. Click again to deselect.
      * Your **total score updates automatically** as you make selections.
      * Receive an assessment of your current stress level and personalised recommendations.

      > The Holmes-Rahe Life Stress Inventory is a screening tool and does not replace a professional clinical evaluation.
    contentAfterInstructions: |-
      <h2 class="no-print">Holmes-Rahe Life Stress Inventory</h2>

      Select each life event that has happened to you during the **previous 12 months**.
    questions:
      - text: 1. Death of spouse
        subscale: default
        minScore: 0
        answers:
          - text: Yes, this happened
            value: 100
      - text: 2. Divorce
        subscale: default
        answers:
          - text: Yes, this happened
            value: 73
      - text: 3. Marital separation from mate
        subscale: default
        answers:
          - text: Yes, this happened
            value: 65
      - text: 4. Detention in jail or other institution
        subscale: default
        answers:
          - text: Yes, this happened
            value: 63
      - text: 5. Death of a close family member
        subscale: default
        answers:
          - text: Yes, this happened
            value: 63
      - text: 6. Major personal injury or illness
        subscale: default
        answers:
          - text: Yes, this happened
            value: 53
      - text: 7. Marriage
        subscale: default
        answers:
          - text: Yes, this happened
            value: 50
      - text: 8. Being fired at work
        subscale: default
        answers:
          - text: Yes, this happened
            value: 47
      - text: 9. Marital reconciliation with mate
        subscale: default
        answers:
          - text: Yes, this happened
            value: 45
      - text: 10. Retirement from work
        subscale: default
        answers:
          - text: Yes, this happened
            value: 45
      - text: 11. Major change in the health or behaviour of a family member
        subscale: default
        answers:
          - text: Yes, this happened
            value: 44
      - text: 12. Pregnancy
        subscale: default
        answers:
          - text: Yes, this happened
            value: 40
      - text: 13. Sexual difficulties
        subscale: default
        answers:
          - text: Yes, this happened
            value: 39
      - text: 14. Gaining a new family member (birth, adoption, older adult moving in, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 39
      - text: 15. Major business readjustment
        subscale: default
        answers:
          - text: Yes, this happened
            value: 39
      - text: 16. Major change in financial state
        subscale: default
        answers:
          - text: Yes, this happened
            value: 38
      - text: 17. Death of a close friend
        subscale: default
        answers:
          - text: Yes, this happened
            value: 37
      - text: 18. Changing to a different line of work
        subscale: default
        answers:
          - text: Yes, this happened
            value: 36
      - text: 19. Major change in the number of arguments with spouse (regarding child rearing, personal habits, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 35
      - text: 20. Taking on a mortgage (for home, business, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 31
      - text: 21. Foreclosure on a mortgage or loan
        subscale: default
        answers:
          - text: Yes, this happened
            value: 30
      - text: 22. Major change in responsibilities at work (promotion, demotion, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 29
      - text: 23. Son or daughter leaving home (marriage, attending college, joined military)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 29
      - text: 24. In-law troubles
        subscale: default
        answers:
          - text: Yes, this happened
            value: 29
      - text: 25. Outstanding personal achievement
        subscale: default
        answers:
          - text: Yes, this happened
            value: 28
      - text: 26. Spouse beginning or ceasing work outside the home
        subscale: default
        answers:
          - text: Yes, this happened
            value: 26
      - text: 27. Beginning or ceasing formal schooling
        subscale: default
        answers:
          - text: Yes, this happened
            value: 26
      - text: 28. Major change in living conditions (new home, remodelling, deterioration of neighbourhood or home, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 25
      - text: 29. Revision of personal habits (dress, manners, associations, quitting smoking)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 24
      - text: 30. Troubles with the boss
        subscale: default
        answers:
          - text: Yes, this happened
            value: 23
      - text: 31. Major changes in working hours or conditions
        subscale: default
        answers:
          - text: Yes, this happened
            value: 20
      - text: 32. Changes in residence
        subscale: default
        answers:
          - text: Yes, this happened
            value: 20
      - text: 33. Changing to a new school
        subscale: default
        answers:
          - text: Yes, this happened
            value: 20
      - text: 34. Major change in usual type or amount of recreation
        subscale: default
        answers:
          - text: Yes, this happened
            value: 19
      - text: 35. Major change in religious activity (a lot more or a lot less than usual)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 19
      - text: 36. Major change in social activities (clubs, movies, visiting, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 18
      - text: 37. Taking on a loan (car, TV, freezer, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 17
      - text: 38. Major change in sleeping habits (a lot more or a lot less than usual)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 16
      - text: 39. Major change in number of family get-togethers
        subscale: default
        answers:
          - text: Yes, this happened
            value: 15
      - text: 40. Major change in eating habits (a lot more or less food intake, or very different meal hours or surroundings)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 15
      - text: 41. Vacation
        subscale: default
        answers:
          - text: Yes, this happened
            value: 13
      - text: 42. Major holidays
        subscale: default
        answers:
          - text: Yes, this happened
            value: 12
      - text: 43. Minor violations of the law (traffic tickets, jaywalking, disturbing the peace, etc.)
        subscale: default
        answers:
          - text: Yes, this happened
            value: 11
    results:
      - resultSummary: Low Risk
        text: There has been a relatively low amount of life change and a low susceptibility to stress-induced health breakdown.
        minScore: 0
        maxScore: 150
        subscale: default
      - resultSummary: Risky
        text: Implies about a 50% risk of a major health breakdown in the next 2 years according to the Holmes-Rahe statistical prediction model.
        minScore: 151
        maxScore: 300
        subscale: default
      - resultSummary: Severe Risk
        text: Implies about an 80% risk of a major health breakdown in the next 2 years according to the Holmes-Rahe statistical prediction model.
        minScore: 301
        maxScore: 1466
        subscale: default
    resultTemplate: 'Your results: {0}'
    bookConsultationButtonText: Book Consultation
    bookConsultationButtonLink: https://cal.com/psychologist-olesia-bobruiko-iudrdc/50-min-consultation
    copyResultsTemplate: |-
      Holmes-Rahe Life Stress Inventory from bobruiko.com on {0}

      RESULT SUMMARY:
      {1} point(s)
      {2}

      INPUTS:
      {3}
    useWhiteForNav: false
    publishTime: 2026-03-20T00:00:00.000Z
    metaDescription: Take the Holmes-Rahe Life Stress Inventory online — assess your stress level based on 43 major life events from the past year and receive personalised recommendations for your mental well-being.
    fbDescription: Take the Holmes-Rahe Life Stress Inventory to assess your stress level based on major life events from the past year and receive personalised recommendations for your mental well-being.
    questionType: checkbox
    noDefaultSelection: true
    copyButtonText: Copy Results
publishTime: 2026-03-20T00:00:00.000Z
---
