...
```javascript
languages: [
  { title: 'UK', icon: '🇺🇦', isDefault: true },
  { title: 'RU', icon: '🇷🇺', isDefault: false },
  { title: 'EN', icon: '🇺🇸', isDefault: false }
]
```
...
```
content/
├── articles_files/          # Blog articles
├── homepage/               # Homepage sections (hero, services, reviews, etc.)
├── pages/                  # Static pages (privacy-policy, про-мене, etc.)
├── questionnaires/         # Psychological questionnaires (GAD-7, PHQ-9, etc.)
├── resources/              # Resources (values, activities catalogue)
└── settings/               # Site configuration (navbar, footer, certificates)
```
...
```
src/
├── config.js              # Global config (API URLs, languages, settings)
...
│   ├── articles/
│   ├── pages/
│   ├── questionnaires/    # Questionnaire functionality
│   ├── resources/
│   └── values/
...
## Build & Deploy
- **Build**: `pnpm build` → generates to `/public`
- **Dev**: `pnpm develop`
- **Deploy**: Netlify (auto-deploy from git)
- **Functions**: Netlify functions in `/netlify/functions/`
- **⚠️ npm install**: Always use `--legacy-peer-deps` flag due to dependency conflicts
